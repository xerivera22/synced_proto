...
export default function TeacherShell() {
  ...
  const lastSubjectId = useMemo(() => {
    try { return localStorage.getItem('teacher.lastSubjectId') || undefined; } catch { return undefined; }
  }, []);

  const searchItems = useMemo(
    () => [
      { id: 'nav-overview', kind: 'Page', title: 'Overview', subtitle: 'Dashboard summary', to: '/teacher/overview' },
      { id: 'nav-subjects', kind: 'Page', title: 'Subjects', subtitle: 'Manage classes and sections', to: '/teacher/subjects' },
      { id: 'nav-class-records', kind: 'Tab', title: 'Class Records', subtitle: 'Grades inside a subject', to: `/teacher/subjects/${lastSubjectId ?? '1'}?tab=class-records` },
      { id: 'nav-attendance', kind: 'Page', title: 'Attendance', subtitle: 'Daily and monthly records', to: '/teacher/attendance' },
      { id: 'nav-schedule', kind: 'Page', title: 'Schedule', subtitle: 'Timetable and events', to: '/teacher/schedule' },
      { id: 'nav-chat', kind: 'Page', title: 'Chat', subtitle: 'Messages with students/parents', to: '/teacher/chat' },
      { id: 'nav-profile', kind: 'Page', title: 'Profile', subtitle: 'Personal information', to: '/teacher/profile' },
      { id: 'nav-settings', kind: 'Page', title: 'Settings', subtitle: 'Preferences', to: '/teacher/settings' },
    ],
    [lastSubjectId]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return searchItems.filter((i) => i.title.toLowerCase().includes(q) || i.subtitle.toLowerCase().includes(q)).slice(0, 8);
  }, [query, searchItems]);
  ...
  return (
    <div data-teacher-ui className="app-container">
      ...
      <div className="md:hidden">
        ...
        />
        {mobileNavOpen && (
          <div className="fixed inset-0 bg-black/30 z-[95]" onClick={() => setMobileNavOpen(false)} />
        )}
      </div>
                      ...
                      filtered.map((item) => (
                        <li key={item.id}>
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (item.id === 'nav-class-records') {
                                const id = ((): string => {
                                  try { return localStorage.getItem('teacher.lastSubjectId') || '1'; } catch { return '1'; }
                                })();
                          ...
                          >
                            ...
                            <div className="text-xs text-gray-500">{item.kind} • {item.subtitle}</div>
                          </button>
                        </li>
                      ))
                    )}
                  ...
                  <div className="profile-menu" role="menu">
                    <div className="profile-meta" aria-hidden="true">
                      ...
                      <div className="profile-texts">
                        <div className="profile-name">{user?.email?.split('@')[0] || 'Teacher'}</div>
