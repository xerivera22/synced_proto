...
export default function TeacherOverview() {
  ...
  return (
    <div className="space-y-3">
      <Banner title="Welcome back, Teacher!" subtitle="Your class tasks today" right={<p className="text-white/80 text-xs md:text-sm whitespace-nowrap">{date}</p>} />
      ...
      <Card className="p-4 bg-[#647FBC]/5 border-[#647FBC]/15">
        ...
        <div className="flex flex-wrap gap-2">
          {['Take Attendance', 'Enter Grades', 'Create Schedule'].map(a => (
            <button key={a} className="px-3 py-2 text-sm border rounded-md hover:bg-gray-50" type="button">{a}</button>
          ))}
        </div>
      </Card>
    </div>
